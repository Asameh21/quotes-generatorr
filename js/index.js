...
function genrateRandomQuote() {
  return quotes[Math.ceil(Math.random() * (quotes.length - 2))];
}

let history = [];

function addToHistory() {
  let theQuote = genrateRandomQuote();
  let newQuote = theQuote.quote;
  if (
    (history.length > 0 && history[history.length - 1] === newQuote) ||
    history.length > 10
  ) {
    history = [];
    return "alert";
...
document.getElementById("new-quote").addEventListener("click", function () {
  let filteredQuote = addToHistory();
  if (filteredQuote !== "alert") {
    document.getElementById("text").innerHTML = filteredQuote.quote;
    document.getElementById("author").innerHTML = filteredQuote.author;
  } else {
...
window.onload = () => {
  document.getElementById("text").innerHTML = quotes[quotes.length - 1].quote;
